...
static dht_result_t dht_compute_data11(uint8_t *data, double *temp, double *humi) {
    *humi = (data[0] << 8 | data[1]) * 0.1;  // Kết hợp 2 byte đầu cho độ ẩm
    *temp = ((data[2] & 0x7F) << 8 | data[3]) * 0.1;  // Kết hợp 2 byte tiếp cho nhiệt độ

    if (data[2] & 0x80) {  // Kiểm tra bit dấu
        *temp = -*temp;
    }

    // In ra giá trị trước khi trả về
    ESP_LOGI("DHT", "Processed: Temp = %.1f, Humi = %.1f", *temp, *humi);
...
dht_result_t read_dht_sensor_data(const gpio_num_t gpio_num, dht_type_t type, struct dht_reading *sensor_data) {
    ...
    ESP_LOGI("DHT", "Raw data: %d %d %d %d %d", data[0], data[1], data[2], data[3], data[4]);
